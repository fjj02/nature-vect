#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# prep-replay-cache.py —— nature-vect cached 绘制引擎缓存预备
# 移植自本地 cell-lct 的 prepare_geometry_cache.py（同作者），仅改名空间
# 为 NATURE_VECT_REPLAY_* 并去掉私有引用。依赖 python3 + fontTools
# （pip install fonttools），纯本地解析、无第三方在线调用。
# 用法见 run_nv_replay.ps1；产出 geometry-cache.json / playback.json。
"""Parse one nature-vect Master SVG once and prepare resumable cached Illustrator replay batches."""
    ...
    for match in TRANSFORM_RE.finditer(value):
        ...
        if name == "matrix" and len(values) == 6:
            ...
            operation = Transform().translate(
                values[0], values[1] if len(values) == 2 else 0.0
            )
        elif name == "scale" and len(values) in {1, 2}:
            operation = Transform().scale(
                values[0], values[1] if len(values) == 2 else values[0]
            )
...
def validate_tree(root: ET.Element) -> None:
    for element in root.iter():
        ...
        if name in FORBIDDEN:
            raise ValueError(
                f"Unsupported SVG element <{name}>; expand it to solid-color paths first"
            )
        if name == "style" or element.get("class"):
            raise ValueError(
                "Embedded or class-dependent CSS is unsupported; inline all presentation attributes"
            )
        for attr_name, attr_value in element.attrib.items():
            lowered = attr_value.lower()
            attr = local_name(attr_name)
            if attr in {"clip-path", "mask", "filter"} and attr_value not in {
                "",
                "none",
            }:
            ...
            if "url(" in lowered or (attr.endswith("href") and attr_value):
                raise ValueError(
                    f"Unsupported linked paint or resource on {element.get('id', name)}"
                )
            if attr == "vector-effect" and attr_value not in {
                "",
                "none",
                "non-scaling-stroke",
            }:
                raise ValueError(
                    "Unsupported vector-effect; expand it before cached playback"
                )
...
def element_path_data(element: ET.Element) -> str:
    ...
    if name in {"polyline", "polygon"}:
        values = [
            float(number) for number in NUMBER_RE.findall(element.get("points", ""))
        ]
        if len(values) < 4 or len(values) % 2:
            raise ValueError(f"Invalid points on <{name}>")
        commands = [f"M {values[0]} {values[1]}"]
        commands.extend(
            f"L {values[index]} {values[index + 1]}"
...
def parse_color(value: str, current_color: str) -> tuple[list[int] | None, float]:
    ...
    if rgb_match:
        components = [
            part.strip()
            for part in re.split(r"[,\s/]+", rgb_match.group(1))
            if part.strip()
        ]
        ...
        if len(components) == 4:
            alpha = (
                float(components[3][:-1]) / 100.0
                if components[3].endswith("%")
                else float(components[3])
            )
...
def paint_parts(
    presentation: dict[str, str], accumulated_opacity: float, stroke_scale: float
) -> list[dict]:
    dash = presentation.get("stroke-dasharray", "none").strip().lower()
    if dash not in {"", "none"}:
        raise ValueError(
            "Dashed strokes must be expanded to solid paths before cached playback"
        )
    current_color = presentation.get("color", "black")
    fill_color, fill_alpha = parse_color(
        presentation.get("fill", "black"), current_color
    )
    stroke_color, stroke_alpha = parse_color(
        presentation.get("stroke", "none"), current_color
    )
    fill_opacity = (
        accumulated_opacity
        * opacity_value(presentation.get("fill-opacity"))
        * fill_alpha
    )
    stroke_opacity = (
        accumulated_opacity
        * opacity_value(presentation.get("stroke-opacity"))
        * stroke_alpha
    )
    ...
    if not filled and not stroked:
        return []
    non_scaling_stroke = (
        presentation.get("vector-effect", "none").strip().lower()
        == "non-scaling-stroke"
    )
    common = {
        "fillRule": "evenodd"
        if presentation.get("fill-rule", "nonzero").strip().lower() == "evenodd"
        else "nonzero",
        "strokeWidth": rounded(
            (parse_number(presentation.get("stroke-width"), 1.0) or 1.0)
            * (1.0 if non_scaling_stroke else stroke_scale)
        ),
        ...
        "strokeMiterLimit": rounded(
            parse_number(presentation.get("stroke-miterlimit"), 4.0) or 4.0
        ),
    }
...
def parse_atom(
    ...
    if local_name(element.tag) == "text":
        ...
        text_opacity = (
            opacity * opacity_value(presentation.get("fill-opacity")) * fill_alpha
        )
        ...
        font_size = (
            parse_number(presentation.get("font-size"), 16.0) or 16.0
        ) * text_scale
        letter_spacing = (
            parse_number(presentation.get("letter-spacing"), 0.0) or 0.0
        ) * text_scale
        font_family = (
            presentation.get("font-family", "Arial")
            .split(",", 1)[0]
            .strip()
            .strip("'\"")
            or "Arial"
        ...
        return {
            ...
            "index": index,
            "sourceId": source_id,
            "objectName": f"NATURE_VECT_REPLAY_ATOM_{index:06d}",
            "text": {
                "contents": content,
                ...
                "dominantBaseline": presentation.get("dominant-baseline", "alphabetic")
                .strip()
                .lower(),
                "letterSpacing": rounded(letter_spacing),
                "rotationDegrees": rounded(
                    math.degrees(math.atan2(transform.xy, transform.xx))
                ),
...
def collect_atoms(root: ET.Element) -> list[dict]:
    ...
    def walk(
        ...
        for child in list(parent):
            ...
            child_hidden = (
                hidden or local.get("display", "inline").strip().lower() == "none"
            )
            child_hidden = child_hidden or child_presentation.get(
                "visibility", "visible"
            ).strip().lower() in {"hidden", "collapse"}
            child_opacity = opacity * opacity_value(local.get("opacity"))
            child_transform = transform.transform(
                parse_transform(child.get("transform"))
            )
            if name in ALLOWED_ATOMS:
                ...
                raise ValueError(
                    f"Unsupported rendered SVG element <{name}>; expand it to a path"
                )
...
def build_batches(
    ...
    if total_atoms < min_size:
        ...
        for atom in atoms:
            ...
            complex_prefix.append(
                complex_prefix[-1] + int(complexity >= complex_threshold)
            )
        ...
        for start in range(total_atoms):
            ...
            if int(atoms[start]["complexity"]) >= complex_threshold:
                relax(
                    start + 1,
                    (current_score[0], current_score[1], current_score[2] + 1),
                    start,
                    "complex",
                )
        ...
        if scores[total_atoms] is None:
            raise ValueError(
                "Could not partition SVG atoms into the required 20-50 playback batches"
            )
        ...
        batches.append(
            {
                "index": batch_index,
                "kind": kind,
                "group_name": f"NATURE_VECT_REPLAY_{job_id}_{batch_index:04d}",
...
def validate_batch_contract(
    ...
    if total_atoms < min_size:
        if (
            len(batches) != 1
            or batches[0]["kind"] != "normal"
            or batches[0]["atomic_count"] != total_atoms
        ):
            raise ValueError(
                "A sub-minimum whole job must remain one ordinary playback batch"
            )
        return
    for batch in batches:
        ...
        if kind == "complex":
            ...
            if int(atoms[atom_index]["complexity"]) < complex_threshold:
                raise ValueError(
                    "Only a genuinely complex atom may be a singleton batch"
                )
            continue
        if not min_size <= count <= max_size:
            raise ValueError(
                f"Ordinary batch size {count} is outside {min_size}..{max_size}"
            )
...
def prepare(
    ...
    if cache_path.exists() or state_path.exists():
        if not cache_path.exists() or not state_path.exists():
            raise ValueError(
                "Cached playback state is incomplete; use a new work directory"
            )
        ...
        if not compatible:
            raise ValueError(
                "Existing geometry cache belongs to different input or batch settings; use a new work directory"
            )
        return cache, state
    ...
    batches = build_batches(
        atoms, job_id, min_size, max_size, complex_threshold, max_points
    )
    ...
    state = {
        ...
        "job_id": job_id,
        "root_group_name": f"NATURE_VECT_REPLAY_JOB_{job_id}",
        "total_atoms": len(atoms),
        "batches": [
            {
                "index": batch["index"],
