// illustrator-replay-runtime.jsx —— nature-vect cached 绘制引擎运行时（Illustrator 内执行）
// 移植自本地 cell-lct 的 cell_lct_cached_runtime.jsx（同作者），配置变量改名
// NATURE_VECT_REPLAY_CONFIG，其余逻辑与源一致。
// 由 run_nv_replay.ps1 通过 DoJavaScript 注入配置调用；操作：draw / save / export。
// - draw  ：按 batchJson（含 viewBox + atoms）把路径/复合路径/文字原子原生画到当前
//           目标文档；文字原子以 live TextFrame（可编辑文本对象）创建。
// - save  ：把目标文档另存为 .ai（或原位保存）。
// - export：把目标文档导出一次 PNG24。
// 对位以画布实际落点为准；完成后 agent 在 AI 内目视微调文字即可。
(function () {
  if (typeof NATURE_VECT_REPLAY_CONFIG === "undefined") {
    return "ERROR|Missing NATURE_VECT_REPLAY_CONFIG";
  }

  var config = NATURE_VECT_REPLAY_CONFIG;
  ...
  function readJson(path) {
    ...
    // The cache is generated locally by prep-replay-cache.py, so parsing
  ...
  function writeGeometry(pathData, destination, viewBox, scale, originLeft, originTop, forceMode) {
    ...
    // Illustrator 2019 is most reliable when ordinary short paths use
    // setEntirePath, while longer paths are appended incrementally.
