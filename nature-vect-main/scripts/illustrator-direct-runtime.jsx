// illustrator-direct-runtime.jsx -- nature-vect direct-replay runtime (Illustrator 2019+).
// Direct engine: Illustrator imports the source SVG itself in a no-UI document,
// samples real PathItem/CompoundPathItem/TextFrame geometry, then natively redraws
// it into the user's currently open document (text becomes live, editable TextFrame).
// Ported from cell-lct (same author) with namespace changes and TextFrame support;
// companion orchestrator: scripts/run_nv_direct.ps1.
(function () {
  if (typeof NATURE_VECT_DIRECT_CONFIG === "undefined") {
    return "ERROR|Missing NATURE_VECT_DIRECT_CONFIG|0|configuration";
  }

  var config = NATURE_VECT_DIRECT_CONFIG;
  var sourceDoc = null;
  ...
  function textFrameGeometry(sourceText) {
    // TextFrame.anchor gives the point-text insertion point in the same
    // document space as the path anchors; fall back to the text bounds.
    var anchor = safeProperty(sourceText, "anchor", null);
    if (anchor === null) {
      try {
        var bounds = sourceText.geometricBounds;
        anchor = [bounds[0], bounds[3]];
      } catch (boundsError) { anchor = [0, 0]; }
    }
    return anchor;
  }

  function snapshotTextFrame(sourceText) {
    var data = {
      contents: safeProperty(sourceText, "contents", ""),
      anchor: textFrameGeometry(sourceText),
      opacity: safeProperty(sourceText, "opacity", 100),
      rotationDegrees: safeProperty(sourceText, "rotation", 0)
    };
    try {
      var attrs = sourceText.textRange.characterAttributes;
      var size = attrs.size;
      data.fontSize = (typeof size === "number" && size > 0) ? size : 16;
      try {
        var font = attrs.textFont;
        data.fontFamily = safeProperty(font, "family", null)
          || safeProperty(font, "name", null) || null;
        data.fontName = safeProperty(font, "name", null);
      } catch (fontError) {}
      try {
        var colorData = attrs.fillColor;
        data.fillColor = (colorData && colorData.typename !== "NoColor")
          ? serializeColor(colorData) : null;
      } catch (colorError) { data.fillColor = null; }
    } catch (attrError) {}
    try {
      var justification = sourceText.textRange.paragraphAttributes.justification;
      if (justification === Justification.CENTER) data.textAnchor = "middle";
      else if (justification === Justification.RIGHT) data.textAnchor = "end";
      else data.textAnchor = "start";
    } catch (justError) { data.textAnchor = "start"; }
    return { type: "TextFrame", text: data };
  }

  function writeTextAppearance(textData, textFrame, scale) {
    try {
      textFrame.contents = String(textData.contents).replace(/\n/g, "\r");
    } catch (contentsError) {}
    try {
      var range = textFrame.textRange;
      var attrs = range.characterAttributes;
      attrs.size = (textData.fontSize || 16) * scale;
      if (textData.fillColor) attrs.fillColor = deserializeColor(textData.fillColor);
      if (textData.fontFamily || textData.fontName) {
        var preferred = textData.fontFamily || textData.fontName;
        try { attrs.textFont = app.textFonts.getByName(preferred); } catch (fontMissing) {}
      }
      if (textData.textAnchor === "middle") range.paragraphAttributes.justification = Justification.CENTER;
      else if (textData.textAnchor === "end") range.paragraphAttributes.justification = Justification.RIGHT;
      else range.paragraphAttributes.justification = Justification.LEFT;
    } catch (attrError) {}
    try { textFrame.opacity = textData.opacity; } catch (opacityError) {}
    if (textData.rotationDegrees) {
      try { textFrame.rotate(-textData.rotationDegrees); } catch (rotationError) {}
    }
  }
  ...
  function collectBottomToTop(container, output) {
    for (var itemIndex = container.pageItems.length - 1; itemIndex >= 0; itemIndex -= 1) {
      ...
      if (itemType === "GroupItem" && !safeProperty(item, "clipped", false)) {
        ...
        output.push(snapshotTextFrame(item));
  ...
  function createAtomic(atomicData, destination, sourceBounds, scale, targetLeft, targetTop) {
    var created;
    if (atomicData.type === "TextFrame") {
      var textAnchor = mapPoint(atomicData.text.anchor, sourceBounds, scale, targetLeft, targetTop);
      var textStagingParent = destination.parent;
      created = textStagingParent.textFrames.pointText(textAnchor);
      try {
        writeTextAppearance(atomicData.text, created, scale);
        created.move(destination, ElementPlacement.PLACEATEND);
      } catch (textError) {
        try { created.remove(); } catch (textCleanupError) {}
        throw textError;
      }
    } else if (atomicData.type === "PathItem") {
  ...
  try {
    ...
    if (!batchMode) {
      ...
      validationGroup.name = "NATURE_VECT_NON_UI_PREFLIGHT";
