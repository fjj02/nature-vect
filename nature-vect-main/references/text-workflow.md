...
## 3. 去字：客户端自带图像能力清掉文字（最关键，必做）

**必做步骤，不得跳过、不得反向索要底图。** 用客户端**自带图像能力**把 manifest 里覆盖的文字区域清除，输出 `clean.png`：

| 客户端 | 去字手段（agent 自动调用） |
|---|---|
| 豆包桌面端 | 自带「像素级覆盖字」：对每块文字 `source_bbox`（可外扩 2~4px 处理残边）逐块做像素级覆盖清除，画布尺寸不变 |
| Codex / 其它 | 各自的图像编辑/局部覆盖工具；不得引入第三方在线清理服务 |

**操作口径（对齐 cell_su7）：**
1. **以原图为参照**，单独产出 `clean.png`，不要在原图上直接改。
2. 推荐提示词模板（整图可用，局部块也可改成逐块口径）：“仅删除所有文字、字母、数字与标注字符，并修复其覆盖的小范围背景。其余图形、图标、曲线、箭头、连线、边框、纹理、颜色、布局、比例、画布尺寸与裁切完全保持原样。不要重画或优化、美化、移动、新增任何非文字元素。”**箭头与图例色块即使紧挨文字也要保留。**

**红线（违反会让产物对位失败）：**
1. **必须保持与去字前同尺寸、同构图**。禁止整图重画、禁止改画布大小/比例、禁止移动非文字元素——只能覆盖/清除文字区域（像素级覆盖属合法手段）。
2. 只有整图重画类生图、没有覆盖能力的客户端：**不要用它**。改用能指定区域的方式，或向用户说明该端无法去字、建议换有覆盖能力的端（豆包）或让用户提供无字底图。
3. **模型处理 ≠ 保真**：清除后把 clean.png 与原图**整体目视对比**，必要时放大文字区复查——查残留字形、图形是否被改、连接线/箭头是否断、画布/对齐有无漂移。改动图形就先修正受影响区域，再进入识别。
4. **第 ④ 步把 clean.png 给用户过目**，确认去字干净且构图未变后再转矢量。

去字效果不足（背景复杂留痕）：如实告知用户，提供选项——接受小瑕疵直接转换，或换一张无字底图（此时把用户给的无字图当 clean.png）。**不得静默跳过文字恢复。** 像素级覆盖保尺寸构图，对位最稳，优于整图生图。
...
- **去字瑕疵**：复杂背景留痕会让对应矢量区域带色块，第 ④ 步闸口把关。
